"""Intake Field Registry for PD3r interview flow.
...
and sequencing information for structured data collection.
...
class IntakeField(BaseModel):
    ...
    conditional: IntakeFieldConditional | None = Field(
        default=None,
...
INTAKE_FIELDS: dict[str, IntakeField] = {
    ...
    for name, config in sorted(
        RAW_INTAKE_FIELDS.items(), key=lambda item: item[1].get("sequence_weight", 0)
    )
}

# Base sequence (always ask first 8 core fields)
BASE_INTAKE_SEQUENCE: list[str] = [
    "position_title",
    "series",
    "grade",
    "organization_hierarchy",
...
SUPERVISORY_ADDITIONAL: list[str] = [
    "supervised_employees",
    "f1_program_scope",
    "f2_organizational_setting",
    "f3_supervisory_authorities",
...
def get_intake_sequence(is_supervisor: bool | None) -> list[str]:
...
# Common federal job series with titles for interview guidance and validation.
class JobSeriesEntry(BaseModel):
    """Pydantic model for a job series entry."""

    code: str = Field(..., description="4-digit job series code")
    title: str = Field(..., description="Official series title")


RAW_JOB_SERIES: dict[str, str] = {
...
JOB_SERIES: dict[str, JobSeriesEntry] = {
    code: JobSeriesEntry(code=code, title=title) for code, title in RAW_JOB_SERIES.items()
}
