...
3. Generation order respects tiers (literal → llm)
"""
...
class TestPrerequisitesConfiguration:
    ...
    def test_llm_sections_require_interview_fields(self):
        """LLM tier sections should require interview data fields."""
        ...
        for section_id in llm_sections:
            ...
            assert len(requires) > 0, (
                f"LLM section {section_id} should have requirements"
            )
...
class TestTierExecutionOrder:
    ...
    def test_tier_priority_ordering(self):
        """Verify tier priority: literal < llm."""
        tier_priority = {"literal": 0, "llm": 1}
    ...
    def test_batch_sections_have_expected_tiers(self):
        """Sections in batches have expected tier assignments."""
        batches = get_drafting_batches({})
        ...
        for batch_name, sections in batches.items():
            ...
            if batch_name == "introduction_duties":
                assert "llm" in tiers  # all sections are LLM
...
class TestCostReductionVerification:
    ...
    def test_llm_call_reduction_potential(self):
        ...
        assert literal_count + llm_count == total_sections

        # Literal sections bypass LLM entirely
        reduction_pct = (literal_count / total_sections) * 100

        # Should bypass at least 10% of sections (2 out of ~11)
        assert reduction_pct >= 10, (
            f"Expected at least 10% bypass, got {reduction_pct:.1f}%"
        )
        ...
        print(f"  Potential bypass: {literal_count} ({reduction_pct:.1f}%)")
